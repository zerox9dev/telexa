...
async function callEdgeFunction(body: Record<string, unknown>) {
  ...
  if (!session?.access_token) throw new Error('Ви не авторизовані')
  ...
  if (!data.ok) throw new Error(data.error || 'Помилка серверної функції')
  return data
}

/** Connect a channel via the server-side bot */
export async function connectChannel(channelUsername: string) {
  if (!isSupabaseConfigured) throw new Error('Supabase не налаштовано')
